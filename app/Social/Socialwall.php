...
class Socialwall extends Database{
    ...
    * Instagram Settings
    **/
    private function setInstagram($client_id = 'client id instagram', $client_secret = 'client secret instagram'){
        ...
        if ($json) {
            ...
            foreach($datas->data as $obj) {
                ...
                $likes          = "";
                $rt             = "";
                if($obj->images && $obj->images->standard_resolution) $img = $obj->images->standard_resolution->url;
    ...
    **/
    private function setTwitter(){
        $consumerKey        = 'consumer key twitter';
        $consumerSecret     = 'consumer secret twitter';
        $accessToken        = 'access token twitter';
        $accessTokenSecret  = 'access token secret twitter';
        ...
        foreach($tweets->statuses as $obj) {
            ...
            if(!empty($obj->entities->urls[0]->url)) $link = $obj->entities->urls[0]->url;
    ...
    **/
    private function setFacebook(){
            ...
            $appSecret  = 'app secret facebook';
            $appToken   = 'app token facebook';

            $_SESSION['fb_token'] = $appToken;

            $connect    = new FacebookConnect($appId, $appSecret);
            $user       = $connect->connect('url connect facebook');
